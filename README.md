# ObsidiaNote · Docusaurus 并行试点站

Obsidian 知识库（ObsidiaNote）的 **Docusaurus 并行试点站**（8084）。用于评估 Docsify → Docusaurus 迁移的可行性，与 Docsify 站（8081）并存对比，**vault 完全只读、零拷贝、零改动**。
...
├── index.html  scripts/                         ← Docsify 站（8081）保留不动
...
- **双 docs 实例直读 vault**：`id: 'tech'` → `vault/Tech`、`id: 'assets'` → `vault/Assets`，`routeBasePath` 保持与 Docsify 一致的 URL 形态（`/Tech/...`、`/Assets/...`）。
...
- systemd `docusaurus-deploy.timer`：每 5 分钟触发，与 ai-assets-site / obsidia 部署共享 `/run/vault-deploy.lock` 串行化 git 操作；以 vault HEAD 状态文件做增量，内容无更新时跳过构建。
...
- 个别 vault 内相对 markdown 链接（`./xxx.md`）Docusaurus 无法解析（Docsify 能），构建期告警，不影响渲染。
